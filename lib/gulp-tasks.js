...
module.exports = function(gulpManifest) {
	...
	amCopy 		= require(__dirname + '/copy.js')(manifest)
	amRewrite 	= require(__dirname + '/rewrite.js')(manifest)
	amAWS 		= require(__dirname + '/aws.js')(manifest)
	amImages	= require(__dirname + '/images.js')(manifest)
	return exports
}
...
function deploy(pkgIndex, destIndex) {
	...
	task +=	"\t amDeploy.deploy(" + pkgIndex + ", " + destIndex + ", done)\n"
	task += "})\n\n"
		...
		if(es6Glob !== "[]") {
			...
			task += "\t es6Srcs.forEach(function(fileGlob, index, array){\n"
				task += "\t\t glob.sync(fileGlob).forEach(function(file, index, array){\n"
					task += "\n"
					task += "\t\t\t var destRelative = path.relative(src, file).replace(/\\/?[^\\/]+$/, '')\n"
					task += "\n"
