...
function copy(manifest, pkgIndex, destIndex) {
	...
	task +=	"\t amCopy.copy(manifest, " + pkgIndex + ", " + destIndex + ", done)\n"
	task += "})\n\n"
...
function images(manifest, pkgIndex, destIndex) {
	...
	task +=	"\t amImages.images(manifest, " + pkgIndex + ", " + destIndex + ", done)\n"
	task += "})\n\n"
...
function deploy(manifest, pkgIndex, destIndex) {
	...
	task +=	"\t amAWS.deploy(manifest, " + pkgIndex + ", " + destIndex + ", done)\n"
	task += "})\n\n"
	...
	if(manifest[pkgIndex]["src"]["js"]) {
		...
		if(Array.isArray(js) && js.length > 0) {
			js.forEach(function(glob, i, a){
				jsGlob += "'" + base + glob.replace(/\\/g, '/') + "',"
			})
			jsGlob = jsGlob.slice(0, -1)
		}
		else if (js !== "") {
			jsGlob += "'" + base + js.replace(/\\/g, '/') + "'"
		}
	}
	jsGlob += "]"
	...
	if(manifest[pkgIndex]["src"]["es6"]) {
		...
		if(Array.isArray(es6) && es6.length > 0) {
			es6.forEach(function(glob, i, a){
				es6Glob += "'" + base + glob.replace(/\\/g, '/') + "',"
			})
			es6Glob = es6Glob.slice(0, -1)
		}
		else if (es6 !== "") {
			es6Glob += "'" + base + es6.replace(/\\/g, '/') + "'"
		}
	}
	es6Glob += "]"
	...
	if(manifest[pkgIndex]["src"]["css"]) {
		...
		if(Array.isArray(css) && css.length > 0) {
			css.forEach(function(glob, i, a){
				cssGlob += "'" + base + glob.replace(/\\/g, '/') + "',"
			})
			cssGlob = cssGlob.slice(0, -1)
		}
		else if (css !== "") {
			cssGlob += "'" + base + css.replace(/\\/g, '/') + "'"
		}
	}
	cssGlob += "]"
	...
	if(manifest[pkgIndex]["src"]["scss"]) {
		...
		if(Array.isArray(scss) && scss.length > 0) {
			scss.forEach(function(glob, i, a){
				scssGlob += "'" + base + glob.replace(/\\/g, '/') + "',"
			})
			scssGlob = scssGlob.slice(0, -1)
		}
		else if (scss !== "") {
			scssGlob += "'" + base + scss.replace(/\\/g, '/') + "'"
		}
	}
	scssGlob += "]"
	...
	manifest.forEach(function(pkg, pkgIndex, array){		
		...
		if(src.js) {			
			if(Array.isArray(src.js) && src.js.length > 0){
				for(var i in src.js) {
					task += "\t gulp.watch('" + base + src.js[i].replace(/\.js/, '*') + "', " + scriptsTasks + ")\n"
				}
			}
			else {
				if(src.js !== "") task += "\t gulp.watch('" + base + src.js.replace(/\.js/, '*')  + "', " + scriptsTasks + ")\n"
			}			
		}
		...
		if(src.es6) {			
			if(Array.isArray(src.es6) && src.es6.length > 0){
				for(var i in src.es6) {
					task += "\t gulp.watch('" + base + src.es6[i].replace(/\.js/, '*')  + "', " + sciptsTasks + ")\n"
				}
			}
			else {
				if(src.es6 !== "") task += "\t gulp.watch('" + base + src.es6.replace(/\.js/, '*')  + "', " + scriptsTasks + ")\n"
			}			
		}
		...
		if(src.css) {			
			if(Array.isArray(src.css) && src.css.length > 0){
				for(var i in src.css) {
					task += "\t gulp.watch('" + base + src.css[i].replace(/\.css/, '*')  + "', " + stylesTasks + ")\n"
				}
			}
			else {
				if(src.css !== "") task += "\t gulp.watch('" + base + src.css.replace(/\.css/, '*')  + "', " + stylesTasks + ")\n"
			}			
		}
		...
		if(src.scss) {			
			if(Array.isArray(src.scss) && src.scss.length > 0){
				for(var i in src.scss) {
					task += "\t gulp.watch('" + base + src.scss[i].replace(/\.scss/, '*')  + "', " + stylesTasks + ")\n"
				}
			}
			else {
				if(src.scss !== "") task += "\t gulp.watch('" + base + src.scss.replace(/\.scss/, '*')  + "', " + stylesTasks + ")\n"
			}			
		}
