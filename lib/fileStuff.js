...
function openModificationsRecord() {
	if(exports.fileExists(__dirname + '/../modificationsRecord.json')) return require(__dirname + '/../modificationsRecord.json')
	else return {}
}

function closeModificationsRecord(record) {
	var output = JSON.stringify(record)
	fs.writeFileSync(__dirname + '/../modificationsRecord.json', output)
}
...
function fileIsNewer(filepath, compareTo) {
	...
	var lastModified = exports.getLastModifiedTime(filepath)
...
function fileNeedsUpdating(record, fileSrc, fileDest) {
	...
	var lastModified = exports.getLastModifiedTime(fileSrc)
	var destExists = exports.fileExists(fileDest)
...
function mkdirIfNotExist(file) {
	...
	while(file.length > 0 && !exports.fileExists(file)) {
